...
<details>
  ...
  2. **Save the Script:**
     - Save the `fn_respawnsectorControl_independentEast_1.sqf` script in your mission's `arma3mercenaries\sector_control folder.
  ...
  3. **Configure the Sector Control Module:**
     ...
     ```sqf
     [_this select 0, _this select 1, _this select 2] execVM 'arma3mercenaries\sector_control\fn_respawnsectorControl_independentEast_1.sqf';
     ```
...
private _message = format ["Sector Seized by %1<br/>Sector Loctaion: %2<br/>Time Captured: %3:%4", _factionName, mapGridPosition _sector, _hours, _minutes];
